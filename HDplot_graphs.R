...
# Modification by Ryan Zhao to support assigning graph saving directory
...
parser$add_argument("-i", "--input", type="character", default='test.depthsBias', required=TRUE,
                    help="Name of the file with extension .depthsBias")
parser$add_argument("-s", "--save_dir", type="character", default='', required=FALSE,
                    help="Graph saving directory")
...
if (minD != maxD) {
  ...
  ggsave(file.path(args$save_dir,"graph1_D_vs_Het.png"), plot=DvH, width = 8, height = 8)
...
if (minN != maxN) {
  ...
  ggsave(file.path(args$save_dir, "graph2_NumSamplesInLocus_vs_Het.png"), plot=NvH, width = 8, height = 8)
  ...
  ggsave(file.path(args$save_dir,"graph3_D_vs_NumSamplesInLocus.png"), plot=DvN, width = 8, height = 8)
...
ggsave(file.path(args$save_dir,"graph4_AlleleRatio_vs_Het.png"), plot=RvH, width = 8, height = 8)
